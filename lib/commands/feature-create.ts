...
import { createAppFeature, updateAppFeature, type AppFeature, type AppFeaturePermissions } from "../api.ts";
...
export const featureCreateCommand = new Command("create")
  ...
  .action(async (options: { name: string; subdomain: string; path: string; devCommand: string; buildCommand: string; outputDir: string; access?: string; permissions?: string; backendDevCommand?: string; backendBuildCommand?: string; backendStartCommand?: string }) => {
    ...
    const localFeatureConfig = fuseConfig.features?.find((feature) => feature.path === featurePath);
    ...
    let accessPrincipals: import("../api.ts").AppFeatureAccessPrincipal[] | undefined;
    ...
    const needsUpdate = accessPrincipals !== undefined || mergedPermissions !== undefined;
    if (needsUpdate) {
      const updateRequest: { accessPrincipals?: import("../api.ts").AppFeatureAccessPrincipal[]; permissions?: AppFeaturePermissions } = {};
      ...
      try {
        ...
        if (accessPrincipals !== undefined) {
          console.log(`✓ Access principals set: ${accessPrincipals.map(p => p.id ? `${p.type}:${p.id}` : p.type).join(', ') || 'none'}`);
        }
        if (mergedPermissions !== undefined) {
          console.log(`✓ Permissions configured: ${mergedPermissions.items.length} item(s)`);
          for (const item of mergedPermissions.items) {
            console.log(`    - ${formatPermissionItem(item)}`);
          }
        }
      } catch (error) {
        console.error(`Warning: Failed to update feature settings. ${error instanceof Error ? error.message : String(error)}`);
      }
    }
    ...
    if (conflictingFeature) {
      console.error(`Error: Path "${featurePath}" is already used by another feature (${conflictingFeature.id}).`);
      process.exit(1);
    }
    ...
    const existingIndex = fuseConfig.features.findIndex(f => f.id === selectedFeature.id);
    ...
    const hasBackendConfig = options.backendDevCommand || options.backendBuildCommand || options.backendStartCommand;
    if (hasBackendConfig) {
      newFeatureConfig.backend = {
        ...(options.backendDevCommand && { dev: { command: options.backendDevCommand } }),
        ...(options.backendBuildCommand && { build: { command: options.backendBuildCommand } }),
        ...(options.backendStartCommand && { start: { command: options.backendStartCommand } }),
      };
    }
    if (existingIndex >= 0) {
    ...
    if (isFirstFeature && selectedFeature.sub) {
      (fuseConfig as Record<string, unknown>)['firstFeatureSub'] = selectedFeature.sub;
    }

    await writeFile(fuseJsonPath, JSON.stringify(fuseConfig, null, 2), "utf-8");
    ...
    if (newFeatureConfig.backend) {
      if (options.backendDevCommand) console.log(`  Backend dev command: ${options.backendDevCommand}`);
      if (options.backendBuildCommand) console.log(`  Backend build command: ${options.backendBuildCommand}`);
      if (options.backendStartCommand) console.log(`  Backend start command: ${options.backendStartCommand}`);
    }
  });
