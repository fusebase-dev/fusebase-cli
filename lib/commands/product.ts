...
export const productCommand = new Command("product").description("Product project maintenance");
...
export async function runProductUpdate(opts: ProductUpdateOptions): Promise<void> {
  ...
  const summary: {
    ...
    installs: doInstall ? "pending" : "skipped",
      ...
      if (!fuseConfig?.orgId || !fuseConfig?.productId) {
        console.error("Error: fusebase.json must include orgId and productId.");
        process.exit(1);
      }
  ...
  console.log("");
  console.log("✓ fusebase product update finished");
}
