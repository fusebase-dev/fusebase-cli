...
- `isolated-store sql bundle` - Build an isolated SQL migration bundle from app files (`apps[].isolatedStores.sql[]`, `postgres/migrations/manifest.json`); can call Gate `status`, `dryRun`, or `apply --yes` using `GATE_MCP_TOKEN` from `.env`. Optional `rls-manifest.json` forwarding requires `fusebase config set-flag postgres-rls`.
