...
Apps run as the main window. The platform provides an app token via `window.FBS_FEATURE_TOKEN` (with `fbsapptoken` cookie fallback when needed).
<% } else { %>
...
<% if (it.flags?.includes("portal-specific-apps")) { %>
1. Read app token on app load from `window.FBS_FEATURE_TOKEN` first; if missing, fall back to `fbsapptoken` cookie
<% } else { %>
1. Read app token on app load: check `fbsapptoken` cookie first, fall back to `window.FBS_FEATURE_TOKEN` if the cookie is absent
<% } %>
...
4. For calls to the app's own backend (`/api/*`), rely on the same-origin cookie and make backend handlers read `x-app-feature-token` or fallback to `fbsapptoken`
