...
For one-click client onboarding into AI Apps, load **`references/app-magic-links.md`**, **`references/fusebase-auth.md`**, and **`app-backend/SKILL.md`** (§ Magic-link session exchange): activation plus **mandatory** app-backend exchange (`POST /api/account/from-magic-link` with `featureToken` + `sessionToken`, Gate via `EverHelper-Session-ID`, app-owned session cookie) before redirect. The bundled `/link` SPA activates only — implement the exchange in app code; do not treat platform cookies as durable identity. Optional: `VITE_FUSEBASE_GATE_URL` for custom domains.
<% if (it.flags?.includes("isolated-stores")) { %>
